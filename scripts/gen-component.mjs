...
function parseReply(raw) {
  ...
  // the model often wraps USAGE in markdown backticks/quotes — strip them so we
  // don't inject `<Tag/>` (literal backticks) into the XML.
  const unwrap = (s) => s?.replace(/^[`'"\s]+|[`'"\s]+$/g, "");
  const code = (raw.match(/---CODE---\n([\s\S]*?)\n---END---/) || [])[1];
  return { tag: field("TAG"), type: field("TYPE"), usage: unwrap(field("USAGE")), note: field("NOTE"), code: code?.trim() };
}
...
function injectIntoRoom(roomId, usage) {
  ...
  if (!/^<[A-Za-z][\w-]*[^<>]*\/>$/.test(usage)) throw new Error(`usage is not a single self-closing element: ${usage}`);
...
let undoRoom = null;
let filePath = null;
const written = new Map(); // component path → prior content (null = didn't exist), for clean revert

// undo everything this run touched: the room injection, and every component file
// we wrote (restoring originals we overwrote, removing ones we created — covers a
// tag rename across repair rounds, and never clobbers an existing component).
const revert = () => {
  if (undoRoom) undoRoom();
  for (const [p, prior] of written) {
    if (prior === null) {
...
for (let round = 0; ; round++) {
  ...
  if (!written.has(filePath)) written.set(filePath, existsSync(filePath) ? readFileSync(filePath, "utf8") : null);
  const withHeader = code.includes("@generated by gen-component") ? code : recipeHeader(args, dateISO) + "\n" + code;
...
if (args.commit) {
  ...
  // stage only what we generated — never `git add -A` (would sweep unrelated
  // working-tree changes into the commit).
  const roomFile = resolve(ROOMS_DIR, `${args.room}.room.xml`);
  execSync(`git add ${JSON.stringify(filePath)} ${JSON.stringify(roomFile)}`, { cwd: ROOT });
